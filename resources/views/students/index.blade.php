    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div class="flex items-center gap-2 animate-fade-in stagger-2">
            <button type="button"
                    ...
                    x-data
                    @click="$dispatch('open-student-modal', { mode: 'create' })">
...
<div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5 animate-fade-in stagger-1">
    ...
    <div class="card px-5 py-4 flex items-center gap-3">
        <div class="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0"
            ...
            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>
        </div>
        <div>
    ...
    <div class="card px-5 py-4 flex items-center gap-3">
        <div class="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0"
            ...
            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
        </div>
        <div>
    ...
    <div class="card px-5 py-4 flex items-center gap-3">
        <div class="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0"
            ...
            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
        </div>
        <div>
...
<div class="card animate-fade-in stagger-2"
    ...
    <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 px-5 py-4 border-b border-slate-100">
        <div class="relative flex-1 min-w-0">
            <svg xmlns="http://www.w3.org/2000/svg" class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 pointer-events-none" style="color: #94a3b8;" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
            <input type="search" class="form-control pl-9 pr-4" placeholder="Cari nama, NIS, kelas…" x-model="search">
        </div>
    ...
    <div class="flex items-center justify-between px-5 py-2.5 border-b border-slate-50" style="background: #f8fafc;">
        <p class="text-xs" style="color: #64748b;">
            Menampilkan <span class="font-semibold" style="color: #1e293b;" x-text="filtered.length"></span> dari <span class="font-semibold" style="color: #1e293b;" x-text="students.length"></span> santri
        </p>
    </div>
        ...
        <table class="data-table">
            ...
            <tbody>
                ...
                <template x-for="(student, idx) in filtered" :key="student.id">
                    <tr>
                        <td class="text-center"><span class="text-sm font-medium text-slate-400" x-text="idx + 1"></span></td>
                        <td><span class="text-sm font-mono text-slate-500" x-text="student.nis || '-'"></span></td>
                        <td>
                            <div class="flex items-center gap-3">
                                <div class="avatar avatar-sm" :style="student.gender === 'Perempuan' ? 'background:#fce7f3; color:#db2777;' : 'background:#dbeafe; color:#2563eb;'">
                        ...
                        <td class="text-center">
                            <span class="badge" :class="student.gender === 'Perempuan' ? 'badge-purple' : 'badge-blue'" x-text="student.gender"></span>
                        </td>
                        <td class="text-center">
                            <span class="badge" :class="student.status === 'aktif' ? 'badge-green' : (student.status === 'keluar' ? 'badge-red' : 'badge-amber')" x-text="student.status"></span>
                        </td>
                        <td class="text-center">
                            <div class="flex items-center justify-center gap-1">
                                <button type="button" class="btn btn-ghost btn-icon btn-sm" @click="$dispatch('open-student-modal', { mode: 'edit', student: student })">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                                </button>
                                <form :action="`/students/${student.id}`" method="POST" onsubmit="return confirm('Yakin ingin menghapus?')">
                                    @csrf
                                    @method('DELETE')
    ...
    <div class="modal-overlay" x-show="open" style="display:none;" @click.self="closeModal()">
        <div class="modal-box max-w-2xl w-full mx-4">
            <div class="modal-header">
                <h2 class="text-base font-bold text-slate-800" x-text="mode === 'create' ? 'Tambah Santri Baru' : 'Edit Data Santri'"></h2>
                <button type="button" @click="closeModal()">&times;</button>
            </div>
            <div class="modal-body">
                <form :action="mode === 'create' ? '{{ route('students.store') }}' : `/students/${form.id}`" method="POST">
                    @csrf
                    ...
                    <div class="grid grid-cols-2 gap-4 mb-4">
                        ...
                        <div>
                            ...
                            <input type="date" name="tanggal_lahir" class="form-control" x-model="form.tanggal_lahir" required>
                        </div>
                        <div>
                            <label class="form-label">Tahun Angkatan</label>
                            <input type="text" name="tahun_angkatan" class="form-control" x-model="form.tahun_angkatan" required>
                        </div>
                        <div class="col-span-2">

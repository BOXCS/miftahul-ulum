...
<div class="smodal-overlay" id="deleteModal">
    ...
    {{-- FINGERPRINT ENROLLMENT MODAL — terintegrasi ESP32 via Reverb --}}
    <div x-data="{
        open: false,
        student: null,
        status: 'ready', // ready, requesting, waiting, success, failed
        quality: 0,
        fingerprintId: null,
        errorMessage: '',
        commandId: null,
        ...
        async startEnroll() {
            ...
            try {
                // Kirim request ke server → masukkan ke command queue
                const { data } = await window.axios.post('/api/iot/enroll-request', {
                ...
                // Listen via Echo — tunggu ESP32 selesai
                this._setupListener();
            } catch (e) {
                this.status = 'failed';
                this.errorMessage = e?.response?.data?.message || 'Gagal kirim perintah ke server.';
            }
        },

        _setupListener() {
            if (!window.Echo) {
                console.warn('Echo tidak tersedia');
                return;
            }
            this._teardownListener();
            this.echoChannel = window.Echo.private('iot-admin')
                .listen('.IotScanResult', (e) => {
                    ...
                    if (this.commandId && e.command_id !== this.commandId) return;

                    this.quality = e.quality || 0;
                    if (e.success) {
                        this.status = 'success';
                        showToast?.('success', 'Berhasil', `Sidik jari ${e.student_name} terdaftar (ID #${e.fingerprint_id})`);
                    } else {
                        this.status = 'failed';
                        this.errorMessage = e.message || 'Gagal mendaftar sidik jari.';
                    }
                });
        },
            ...
            <div class="modal-box max-w-md w-full mx-4">
                ...
                <div class="modal-body text-center py-6">
                    ...
                    <div class="flex justify-center mb-6">
                        <div class="w-32 h-32 rounded-full flex items-center justify-center border-4 transition-all"
                            :class="{
                                'border-slate-200 text-slate-400': status === 'ready',
                                'border-amber-500 text-amber-500 animate-pulse': status === 'requesting' || status === 'waiting',
                    ...
                    <div class="mb-4 min-h-[3rem]">
                        <p x-show="status === 'ready'" class="text-slate-600">
                            Tekan tombol di bawah, lalu minta santri menempelkan jari di sensor ESP32.
                        </p>
                        <p x-show="status === 'requesting'" class="text-amber-600 font-medium">
                            Mengirim perintah ke perangkat…
                        </p>
                        <p x-show="status === 'waiting'" class="text-amber-600 font-medium">
                            <span class="block">Letakkan jari pada sensor ESP32…</span>
                            <span class="block text-xs text-amber-700 mt-1">Slot fingerprint ID: <strong x-text="'#' + fingerprintId"></strong></span>
                        </p>
                        <p x-show="status === 'success'" class="text-green-600 font-medium">
                            ✓ Berhasil terdaftar! Fingerprint ID: <span x-text="'#' + fingerprintId"></span>
                            <span x-show="quality" class="block text-xs">Kualitas: <span x-text="quality + '%'"></span></span>
                        </p>
                        <p x-show="status === 'failed'" class="text-red-600 font-medium" x-text="errorMessage"></p>
                    </div>

                    <div x-show="status === 'success' && quality" class="w-full bg-slate-200 rounded-full h-2.5 mb-6">
                        <div class="bg-green-600 h-2.5 rounded-full transition-all" :style="'width: ' + quality + '%'"></div>
                    </div>

                    <div class="flex justify-center gap-3">
                        <button type="button" class="btn btn-primary"
                            @click="startEnroll()"
                            x-show="status === 'ready' || status === 'failed'"
                            :disabled="status === 'requesting' || status === 'waiting'">
                            <span x-text="status === 'failed' ? 'Coba Lagi' : 'Mulai Scan di Sensor'"></span>
                        </button>
                        <button type="button" class="btn btn-secondary cursor-wait"
                            x-show="status === 'requesting' || status === 'waiting'" disabled>
                            Menunggu ESP32…
                        </button>
                    </div>
                </div>
                <div class="modal-footer flex justify-end">
                    <button type="button" class="btn btn-secondary" @click="closeModal()">
                        <span x-text="status === 'success' ? 'Tutup' : 'Batal'"></span>
                    </button>
                </div>
            </div>
        </div>

...
@section('content')
<div class="att-page"></div>

    <div class="toast-wrap" id="toastWrap" data-success="{{ session('success') }}" data-error="{{ session('error') }}">
    </div>
    ...
    <div
        ...
        <div>
            <h1 style="font-size:1.75rem;font-weight:800;color:#0f172a;letter-spacing:-.04em;line-height:1;">Presensi Santri
            </h1>
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;">
            ...
            <div class="flex items-center gap-3">
                <button type="button" class="btn btn-primary bg-indigo-600 hover:bg-indigo-700 text-white"
        ...
        <div class="tcard" x-data="{
            ...
            <div class="result-bar">
                <span>
                    Menampilkan <strong x-text="filtered.length"></strong> dari <strong x-text="students.length"></strong>
                    santri
                    &nbsp;·&nbsp;
                    <span
                        style="color:#0f172a;font-weight:600;">{{ \Carbon\Carbon::parse($today)->isoFormat('dddd, D MMMM Y') }}</span>
                    &nbsp;·&nbsp;
                    <span
                        style="display:inline-flex;align-items:center;gap:4px;background:#ccfbf1;color:#0d9488;font-weight:700;padding:2px 8px;border-radius:6px;"
                        x-text="'Waktu: ' + (waktu_shalat === 'semua' ? 'Semua' : waktu_shalat)"></span>
                </span>
            </div>
                ...
                <table class="dt">
                    <thead>
                        <tr>
                            ...
                            <template x-if="waktu_shalat !== 'semua'">
                                <th class="teal-col" style="text-align:center;min-width:100px;" x-text="waktu_shalat">
                                </th>
                            </template>
                    ...
                    <tbody>
                        <tr x-show="filtered.length === 0">
                            <td :colspan="6 + (waktu_shalat === 'semua' ? prayers.length : 1) + (showJamCols ? 2 : 0)"
                                ...
                                <div
                                    style="width:56px;height:56px;background:#f1f5f9;border-radius:50%;display:flex;align-items:center;justify-content:center;margin:0 auto 10px;font-size:1.5rem;">
                                    📋</div>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div style="display:flex;align-items:center;gap:10px;">
                                        ...
                                        <div>
                                            <span style="font-size:.85rem;font-weight:700;color:#1e293b;display:block;"
                                                x-text="student.name"></span>
                                            <span style="font-size:.72rem;color:#94a3b8;"
                                                x-text="'NIS: ' + (student.nis || '-')"></span>
                                        </div>
                                    </div>
                                </td>
                                <td style="text-align:center;">
                                    <span
                                        style="display:inline-flex;align-items:center;justify-content:center;min-width:38px;height:26px;border-radius:8px;background:#f1f5f9;color:#475569;font-size:.75rem;font-weight:800;padding:0 6px;"
                                            ...
                                            <div style="display:flex;align-items:center;justify-content:center;gap:3px;">
                                                <template x-if="getAtt(student.id, p).status === 'alpha'">
                                                    <span style="position:relative;display:inline-flex;margin-right:2px;">
                                                        <span
                                                            style="position:absolute;display:inline-flex;width:100%;height:100%;border-radius:50%;background:#fca5a5;opacity:.75;animation:ping 1.2s cubic-bezier(0,0,.2,1) infinite;"></span>
                                                        <span
                                                            style="width:7px;height:7px;border-radius:50%;background:#ef4444;display:inline-flex;position:relative;"></span>
                                                    </span>
                                                </template>
                                ...
                                <template x-if="waktu_shalat !== 'semua'">
                                    <td class="teal-cell" style="text-align:center;">
                                        <div style="display:flex;align-items:center;justify-content:center;gap:3px;">
                                            ...
                                            <span class="bdg"
                                                :class="statusClass(getAtt(student.id, waktu_shalat).status)"
                                ...
                                <template x-if="showJamCols">
                                    <td style="text-align:center;">
                                        <span
                                            style="font-size:.75rem;font-family:monospace;color:#475569;background:#f8fafc;padding:2px 8px;border-radius:6px;"
                                ...
                                <td>
                                    <template
                                        x-if="waktu_shalat !== 'semua' && getAtt(student.id, waktu_shalat).keterangan">
                                        <span class="ket-pill"
                                            x-text="getAtt(student.id, waktu_shalat).keterangan"></span>
                                    </template>
                                    <template
                                        x-if="waktu_shalat === 'semua' || !getAtt(student.id, waktu_shalat).keterangan">
            ...
            {{-- Pagination --}}
            <div
                style="padding:12px 20px;border-top:1px solid #f1f5f9;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px;">
                <span style="font-size:.75rem;color:#64748b;">
                    Menampilkan
                    <strong style="color:#0f172a;" x-text="filtered.length === 0 ? 0 : startNumber + 1"></strong>–<strong
                        style="color:#0f172a;" x-text="Math.min(startNumber + perPage, filtered.length)"></strong>
                ...
                <div style="display:flex;align-items:center;gap:5px;flex-wrap:wrap;">
                    ...
                    <template x-for="page in pages" :key="page">
                        <button type="button" @click="goPage(page)"
                            ...
                            :style="currentPage === page ? 'background:#0d9488;color:#fff;border-color:#0d9488;' :
            ...
            <div class="smodal-overlay" :class="{ open: editModal }" @click.self="closeEdit()">
                <div class="smodal-box" style="max-width:500px;">
                    <div
                        style="display:flex;align-items:center;justify-content:space-between;padding:18px 24px 14px;border-bottom:1px solid #f1f5f9;flex-shrink:0;">
                        <div style="display:flex;align-items:center;gap:12px;">
                            <div
                                style="width:36px;height:36px;border-radius:11px;background:#ccfbf1;color:#0d9488;display:flex;align-items:center;justify-content:center;">
                            ...
                            <div>
                                ...
                                <div style="font-size:.72rem;color:#94a3b8;"
                                    x-text="editForm.student_name + ' · ' + editForm.waktu"></div>
                            </div>
                        </div>
                    ...
                    <div class="smodal-body">
                        <form action="{{ route('attendance.store') }}" method="POST" id="editAttForm">
                            ...
                            {{-- Student info strip --}}
                            <div
                                style="display:flex;align-items:center;gap:12px;padding:12px 14px;background:#f0fdfa;border-radius:12px;border:1.5px solid #99f6e4;margin-bottom:20px;">
                                <div style="width:40px;height:40px;border-radius:12px;display:flex;align-items:center;justify-content:center;font-size:.75rem;font-weight:800;color:#fff;flex-shrink:0;"
                                    ...
                                    <span
                                        x-text="editForm.student_name ? editForm.student_name.charAt(0).toUpperCase() : ''"></span>
                                </div>
                                <div>
                                    <div style="font-size:.88rem;font-weight:700;color:#0f172a;"
                                        x-text="editForm.student_name"></div>
                                    <div style="font-size:.72rem;color:#64748b;"
                                        x-text="editForm.waktu + ' · {{ \Carbon\Carbon::parse($today)->isoFormat('D MMMM Y') }}'">
                                    </div>
                                </div>
                            </div>
                            ...
                            <div style="margin-bottom:18px;">
                                <label
                                    style="display:block;font-size:.72rem;font-weight:800;color:#475569;text-transform:uppercase;letter-spacing:.08em;margin-bottom:10px;">
                                    Status Kehadiran <span style="color:#e11d48;">*</span>
                                </label>
                                <div style="display:grid;grid-template-columns:repeat(5,1fr);gap:8px;">
                                    <template x-for="opt in ['hadir','terlambat','izin','sakit','alpha']"
                                        :key="opt">
                                        <label
                                            ...
                                            :style="editForm.status === opt ?
                                                ...
                                                opt==='izin' ?
                                                'border-color:#d97706;background:#fffbeb;color:#b45309;' :
                                                opt==='sakit' ?
                                            ...
                                            <input type="radio"
                                                ...
                                                :value="opt" x-model="editForm.status" style="display:none;">
                                            ...
                                            <span style="font-size:.7rem;text-transform:capitalize;"
                                                x-text="opt"></span>
                                        </label>
                                    </template>
                                </div>
                            ...
                            <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:16px;">
                                <div>
                                    <label
                                        style="display:block;font-size:.72rem;font-weight:800;color:#475569;text-transform:uppercase;letter-spacing:.08em;margin-bottom:7px;">Jam
                                        Masuk</label>
                                    <input type="time" :name="'attendances[' + editForm.student_id + '][jam_masuk]'"
                                ...
                                <div>
                                    <label
                                        style="display:block;font-size:.72rem;font-weight:800;color:#475569;text-transform:uppercase;letter-spacing:.08em;margin-bottom:7px;">Jam
                                        Keluar</label>
                                    <input type="time" :name="'attendances[' + editForm.student_id + '][jam_keluar]'"
                            ...
                            <div>
                                <label
                                    style="display:block;font-size:.72rem;font-weight:800;color:#475569;text-transform:uppercase;letter-spacing:.08em;margin-bottom:7px;">
                                    Keterangan <span
                                        style="font-size:.68rem;font-weight:500;text-transform:none;color:#94a3b8;">(opsional)</span>
                                </label>
                                <textarea :name="'attendances[' + editForm.student_id + '][keterangan]'" rows="2" x-model="editForm.keterangan"
            ...
            <div class="smodal-overlay" :class="{ open: deleteModal }" @click.self="closeDelete()">
                <div class="smodal-box" style="max-width:400px;">
                    <div class="smodal-body" style="text-align:center;padding:32px 24px 20px;">
                        <div
                            style="width:60px;height:60px;border-radius:18px;background:#fee2e2;display:flex;align-items:center;justify-content:center;margin:0 auto 16px;">
                        ...
                        <div style="font-size:1.1rem;font-weight:800;color:#0f172a;margin-bottom:6px;">Hapus Presensi?
                        </div>
                        <p style="font-size:.82rem;color:#64748b;margin-bottom:14px;">Data presensi berikut akan dihapus
                            permanen:</p>
                        <div
                            style="padding:12px 14px;background:#fff7f7;border:1px solid #fecaca;border-radius:12px;text-align:left;">
                            <div style="font-size:.85rem;font-weight:700;color:#0f172a;"
                                x-text="deleteTarget?.student_name"></div>
                            <div style="font-size:.75rem;color:#64748b;margin-top:3px;" x-text="deleteTarget?.waktu">
                            </div>
                        </div>
                    </div>
                    <div class="smodal-ftr">
                        ...
                        <button @click="submitDelete()"
                            ...
                            <div x-data="{
                                        ...
                                        try {
                                            ...
                                            const response = await fetch('/attendance/verify', {
                                                        ...
                                                        headers: {
                                                            ...
                                                            'X-CSRF-TOKEN': document.querySelector('meta[name=\"csrf-token\"]').getAttribute('content') }, body: JSON.stringify({
                                ...
                                this.status = 'success'; this.matchResult=data.student; // Otomatis tandai hadir
                                this.markAttendance(data.student.id); } else { throw new Error(data.message
                                || 'Sidik jari tidak dikenali.' ); } } catch (error) { this.status = 'failed';
                                this.errorMessage=error.message; } }, markAttendance(studentId) { // Submit hidden form
                                otomatis const form=document.createElement('form'); form.method = 'POST';
                                form.action = '{{ route('attendance.store') }}'; const
                                csrfToken=document.createElement('input'); csrfToken.type = 'hidden';
                                csrfToken.name = '_token';
                                csrfToken.value=document.querySelector('meta[name=\"csrf-token\"]').getAttribute('content');
                                form.appendChild(csrfToken); const tanggalInput=document.createElement('input');
                                tanggalInput.type = 'hidden'; tanggalInput.name = 'tanggal';
                                tanggalInput.value=document.querySelector('input[name=\"tanggal\"]').value
                                || '{{ $today }}' ; form.appendChild(tanggalInput); const
                                waktuShalatInput=document.createElement('input'); waktuShalatInput.type = 'hidden';
                                waktuShalatInput.name = 'waktu_shalat';
                                waktuShalatInput.value=document.querySelector('select[name=\"waktu_shalat\"]').value
                                || '{{ $waktu_shalat }}' ; form.appendChild(waktuShalatInput); const
                                statusInput=document.createElement('input'); statusInput.type = 'hidden';
                                statusInput.name = 'attendances[' + studentId + '][status]' ; statusInput.value = 'hadir';
                                form.appendChild(statusInput); document.body.appendChild(form); setTimeout(()=>
                                form.submit(), 1500);
                                }
                                }" @open-verify-modal.window="openModal()">

                                <div class="modal-overlay" x-show="open" style="display:none;"
                                            ...
                                            <div class="flex justify-center mb-6">
                                                <div class="w-24 h-24 rounded-full flex items-center justify-center border-4"
                                                    ...
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-12 h-12"
                                                        ...
                                                        stroke-linejoin="round">
                                                        <path
                                                            d="M12 2a10 10 0 0 0-10 10v2a10 10 0 0 0 10 10 10 10 0 0 0 10-10v-2a10 10 0 0 0-10-10z" />
                                                        <path
                                                            d="M12 6a6 6 0 0 0-6 6v2a6 6 0 0 0 6 6 6 6 0 0 0 6-6v-2a6 6 0 0 0-6-6z" />
                                                        <path
                                                            d="M12 10a2 2 0 0 0-2 2v2a2 2 0 0 0 2 2 2 2 0 0 0 2-2v-2a2 2 0 0 0-2-2z" />
                                                    </svg>
                                                </div>
                                            </div>

                                            <div class="mb-6 h-12">
                                                <p x-show="status === 'ready'" class="text-slate-600">Tekan tombol di
                                                    bawah dan letakkan jari Anda pada scanner</p>
                                                <p x-show="status === 'scanning'" class="text-indigo-600 font-medium">
                                                    Sedang memverifikasi...</p>

                                                <div x-show="status === 'success'" class="text-green-600">
                                                    <p class="font-bold text-lg"
                                                        x-text="matchResult ? matchResult.name : ''"></p>
                                                    <p class="text-sm">Berhasil diverifikasi! Menyimpan absen...</p>
                                                </div>

                                                <p x-show="status === 'failed'" class="text-red-600 font-medium"
                                                    x-text="errorMessage"></p>
                                            </div>

                                            <button type="button" class="btn w-full justify-center"
                                                :class="status === 'scanning' ? 'btn-secondary opacity-50 cursor-not-allowed' :
                                                    'btn-primary'"
                                                @click="scanAndVerify()" :disabled="status === 'scanning'">
                                                <span
                                                    x-text="status === 'success' || status === 'failed' ? 'Coba Lagi' : 'Mulai Scan'"></span>
                                            </button>
                                        </div>
                                    </div>
                                </div>
                        ...
                        @push('scripts')
                            ...
                            <script>
                                function showToast(type, title, msg, duration = 4000) {
                                    ...
                                    t.innerHTML =
                                        `<div class="toast-ico">${icons[type]||icons.success}</div><div><div class="toast-title">${title}</div><div class="toast-msg">${msg}</div></div>`;
                                    wrap.appendChild(t);

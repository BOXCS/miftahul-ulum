                ...
                <div class="smodal-box" style="max-width:400px;">
                    ...
                    <div class="smodal-ftr">
                        ...
                        <button @click="submitDelete()"
                            ...
                            Ya, Hapus
                            {{-- FINGERPRINT VERIFY MODAL — listen real-time scan absensi dari ESP32 --}}
                            <div x-data="{
                                open: false,
                                status: 'ready', // ready, listening, success, failed
                                errorMessage: '',
                                matchResult: null,
                                echoChannel: null,
                                ...
                                _setupListener() {
                                    if (!window.Echo) {
                                        this.status = 'failed';
                                        this.errorMessage = 'Echo/Reverb tidak tersedia.';
                                        return;
                                    }
                                    this._teardownListener();
                                    this.echoChannel = window.Echo.private('iot-admin')
                                        .listen('.IotScanResult', (e) => {
                                            // Hanya tangkap event scan absensi
                                            if (e.kind !== 'attendance') return;
                                            ...
                                            if (e.success) {
                                                ...
                                                this.matchResult = {
                                                    name: e.student_name,
                                                    kelas: e.kelas,
                                                    waktu_shalat: e.waktu_shalat,
                                                    statusHadir: e.status,
                                                    jam: e.jam_scan,
                                                };
                                                showToast('success', 'Absen tercatat', `${e.student_name} — ${e.waktu_shalat} (${e.status})`);
                                                // Auto-refresh halaman setelah 3 detik agar tabel update
                                                setTimeout(() => window.location.reload(), 2500);
                                            } else {
                                                this.status = 'failed';
                                                this.errorMessage = e.message || 'Sidik jari tidak dikenal.';
                                            }
                                        });
                                },
                                    ...
                                    <div class="modal-box max-w-sm w-full mx-4">
                                        ...
                                        <div class="modal-body text-center py-6">
                                            <div class="flex justify-center mb-6">
                                                <div class="w-24 h-24 rounded-full flex items-center justify-center border-4 transition-all"
                                                    :class="{
                                                        'border-slate-200 text-slate-400': status === 'ready',
                                                        'border-indigo-500 text-indigo-500 animate-pulse': status === 'listening',
                                            ...
                                            <div class="mb-6 min-h-[3.5rem]">
                                                <p x-show="status === 'listening'" class="text-indigo-600 font-medium">
                                                    Letakkan jari santri pada sensor ESP32…
                                                    <span class="block text-xs text-indigo-500 mt-1">Web sedang mendengarkan event scan real-time.</span>
                                                </p>

                                                <div x-show="status === 'success'" class="text-green-600">
                                                    <p class="font-bold text-lg" x-text="matchResult?.name"></p>
                                                    <p class="text-sm">
                                                        <span x-text="matchResult?.waktu_shalat"></span> —
                                                        <strong x-text="matchResult?.statusHadir?.toUpperCase()"></strong>
                                                        (<span x-text="matchResult?.jam"></span>)
                                                    </p>
                                                    <p class="text-xs text-green-700 mt-1">Halaman akan refresh otomatis…</p>
                                                </div>
                                            ...
                                            <button type="button" class="btn w-full justify-center btn-primary"
                                                x-show="status === 'failed'"
                                                @click="retry()">
                                                Dengarkan Lagi
                                            </button>
                                            <button type="button" class="btn w-full justify-center btn-secondary"
                                                x-show="status === 'listening'"
                                                @click="closeModal()">
                                                Batal
                                            </button>
                                        </div>
